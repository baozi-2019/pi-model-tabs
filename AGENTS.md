...
- tag 格式：`v<major>.<minor>`（如 `v1.0`、`v1.1`），打在 `dev` 分支当前提交上，使用 annotated tag（中文说明）。紧急修复可用 hotfix 格式 `v<major>.<minor>-fix<YYYYMMDDNN>`（如 `v1.1-fix2026091801`）。
- 用户说「打 tag」而未指定版本时，自动读取最新 tag 递增 **0.1**（如 `v1.0` → `v1.1`）；用户直接指定版本（如「打个 tag 2.0」或 hotfix 名）时按指定值打。
- 递增前确认现有 tag 均符合 `v<major>.<minor>` 格式；无法解析最新 tag 时向用户确认，不臆测。
- 打 tag 后在交付说明中报告新旧版本号；**创建与推送 tag 都必须获得明确授权，未授权仅打本地 tag，不推送**。

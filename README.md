...
```bash
# 更新到指定 tag（推荐，锁定版本）
pi install git:github.com/baozi-2019/pi-model-tabs@v1.1-fix2026091801
...
> ⚠️ 不要用 `Ctrl+M` 等终端控制字符等价键位：`Ctrl+M` 与回车（Enter）在终端中是同一个字节（CR），注册它会劫持回车发送消息；同理避免 `Ctrl+I`（=Tab）、`Ctrl+H`（=Backspace）、`Ctrl+J`（=换行）。
